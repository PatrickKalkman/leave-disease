...
def create_cnn_model():
    ...
    pre_trained_model = tf.keras.applications.ResNet152V2(input_shape=(*IMG_SIZE, 3),
                                    weights='imagenet',
                                    include_top=False)
    ...
    model.compile(loss='categorical_crossentropy', optimizer=tf.optimizers.Adamax(lr=0.01), metrics=['accuracy'])
...
def create_callbacks():
    ...
    reduce_lr = ReduceLROnPlateau(factor = 0.3, monitor='val_loss', min_lr=0.001,
                                  patience=2, mode='min',min_delta = 0.001, 
                                  verbose=1)
...
ld.distribute_images(0.95, CSV_LOCATION, TRAINING_IMAGES_INPUT)
